...
> {
  ...
  const rows = await db
    .select({
      ...
      color: restaurantTables.color,
      seatingType: restaurantTables.seatingType,
      chairLayout: restaurantTables.chairLayout,
  ...
  const data: Table[] = rows.map((r) => ({
    ...
    shape: r.shape,
    status: r.status,
    seatingType: r.seatingType as Table["seatingType"],
    chairLayout: (r.chairLayout as Table["chairLayout"]) ?? undefined,
    color: r.color as Table["color"],
