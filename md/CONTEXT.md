...
40. OPEN, flagged not yet fixed (2026-08-29): features/pos/components/MenuPanel/MenuItemCard.tsx's CATEGORY_ICON fallback map is keyed to hardcoded placeholder category IDs (cat_001 through cat_008). Since every tenant has its own dedicated DB with real generated UUIDs, this will never match in production and always falls back to the generic UtensilsCrossed icon. Categories already carry their own admin-entered icon field (emoji) used correctly elsewhere (CategorySidebar.tsx, CategoryPills.tsx) — MenuItemCard.tsx should likely use item's category's real icon field instead of this hardcoded map. Zakir confirmed yes to fixing — not yet done, pick up next session.
...
Immediate next task (confirmed with Zakir, not yet started): fix Bug Pattern #40 (MenuItemCard.tsx hardcoded CATEGORY_ICON placeholder-ID map) before moving to the next module.