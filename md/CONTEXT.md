...
40. OPEN, still not fixed (2026-08-29, correction added 2026-09-01): originally described as living in features/pos/components/MenuPanel/MenuItemCard.tsx, but that file was checked 2026-09-01 and does NOT contain a CATEGORY_ICON map — it already accepts a categoryIcon prop cleanly. The hardcoded cat_001–cat_008 placeholder-ID map must live in whichever parent component computes and passes that prop down (not yet located — grep for CATEGORY_ICON across features/pos was queued but not completed before the session ended). Next session: run Get-ChildItem -Recurse -Include "*.tsx","*.ts" -Path "src\features\pos" | Select-String -Pattern "CATEGORY_ICON" -List to find the real file, then fix it to use the category's real admin-entered icon field instead of the placeholder-ID map, matching CategorySidebar.tsx/CategoryPills.tsx's existing correct pattern.
